...
const FallingBadge: React.FC<FallingBadgeProps> = ({
  ...
  useEffect(() => {
    ...
    const badgeBodies = [...badgeElements].map(elem => {
      ...
      const isCircle = elem.classList.contains('rounded-full') && rect.width === rect.height

      const x = rect.left - containerRect.left + rect.width / 2
      const y = rect.top - containerRect.top + rect.height / 2

      const body = isCircle
        ? Bodies.circle(x, y, rect.width / 2, {
            render: { fillStyle: 'transparent' },
            restitution: 0.8,
            frictionAir: 0.01,
            friction: 0.2
          })
        : Bodies.rectangle(x, y, rect.width, rect.height, {
            render: { fillStyle: 'transparent' },
            restitution: 0.8,
            frictionAir: 0.01,
            friction: 0.2,
        ...
        {badges.map((badge, index) =>
          badge.type === 'circle' ? (
            <div
              key={index}
              data-badge-item
              className={cn('flex size-12 items-center justify-center rounded-full select-none', badge.className)}
            >
              {badge.icon}
            </div>
          ) : (
